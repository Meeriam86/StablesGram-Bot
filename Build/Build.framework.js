...
var ASM_CONSTS = {
  4069984: function() {var AudioContext = window.AudioContext || window.webkitAudioContext; var infocontext = new AudioContext(); if (!infocontext) { return 0; } var inforate = infocontext.sampleRate; infocontext.close(); return inforate;},  
 4070204: function() {Module.mInputRegistered = false;},  
 4070241: function() {var AudioContext = window.AudioContext || window.webkitAudioContext; Module.context = new AudioContext(); if (!Module.context) { return 0; } Module.FMOD_JS_MixFunction = Module["cwrap"]('FMOD_JS_MixFunction', 'void', ['number']); return Module.context.sampleRate;},  
 4070509: function($0, $1) {Module._as_script_node = Module.context.createScriptProcessor($1, 0, $0); Module["OutputWebAudio_resumeAudio"] = function() { if (Module.context && Module.mInputRegistered) { console.log('Resetting audio driver based on user input.'); Module._as_script_node.connect(Module.context.destination); Module._as_script_node.onaudioprocess = function(audioProcessingEvent) { Module._as_output_buffer = audioProcessingEvent.outputBuffer; Module.FMOD_JS_MixFunction(Module._as_output_buffer.getChannelData(0).length); }; Module.context.resume(); window.removeEventListener('click', Module.OutputWebAudio_resumeAudio, false); window.removeEventListener('touchend', Module.OutputWebAudio_resumeAudio, false); Module.mInputRegistered = false; } };},  
 4071249: function() {if (Module.mInputRegistered) { Module.mInputRegistered = false; window.removeEventListener('click', Module["OutputWebAudio_resumeAudio"], false); window.removeEventListener('touchend', Module["OutputWebAudio_resumeAudio"], false); }},  
 4071486: function() {window.addEventListener('touchend', Module["OutputWebAudio_resumeAudio"], false); window.addEventListener('click', Module["OutputWebAudio_resumeAudio"], false); Module.mInputRegistered = true;},  
 4071683: function() {Module._as_script_node.disconnect(Module.context.destination);},  
 4071750: function($0, $1, $2, $3) {var data = HEAPF32.subarray(($0 / 4), ($0 / 4) + ($2 * $3)); for (var channel = 0; channel < $3; channel++) { var outputData = Module._as_output_buffer.getChannelData(channel); for (var sample = 0; sample < $2; sample++) { outputData[sample+$1] = data[(sample*$3)+channel]; } }},  
 4072032: function() {Module.context.suspend();},  
 4072062: function() {Module.context.resume();},  
 4072091: function() {var AudioContext = window.AudioContext || window.webkitAudioContext; var infocontext = new AudioContext(); if (!infocontext) { return 0; } var inforate = infocontext.sampleRate; infocontext.close(); return inforate;},  
 4072311: function() {Module.mWorkletNode = null; Module.mModulePolling = false; Module.mModuleLoading = false; Module.mStartInterval = null; Module.mStopInterval = null; Module.mSuspendInterval = null; Module.mResumeInterval = null; Module.mWorkletNodeConnected = false; Module.mInputRegistered = false; var AudioContext = window.AudioContext || window.webkitAudioContext; Module.mContext = new AudioContext(); if (!Module.mContext) { return 0; } Module.mContext.destination.channelCount = Module.mContext.destination.maxChannelCount; return Module.mContext.destination.maxChannelCount;},  
 ...
 4079807: function() {Module.mStartInterval = Module["waitForAudioWorklet"](function(){ return (Module.mWorkletNode === null && Module.mModulePolling); }, function(){ if (Module.mWorkletNode) { Module.mWorkletNode.connect(Module.mContext.destination); Module.mWorkletNodeConnected = true; } });},  
 4080084: function() {Module.mStopInterval = Module["waitForAudioWorklet"](function(){ return (Module.mWorkletNode === null && Module.mModulePolling || !Module.mWorkletNodeConnected); }, function(){ if (Module.mWorkletNode) { Module.mWorkletNode.disconnect(); Module.mWorkletNodeConnected = false; } });},  
 4080370: function($0, $1, $2) {const buffer = $0; const bufferLength = $1; const speakerModeChannels = $2; var data = HEAPF32.subarray((buffer / 4), (buffer / 4) + (bufferLength * speakerModeChannels)); for (var channel = 0; channel < speakerModeChannels; channel++) { const offset = channel * bufferLength; for (var sample = 0; sample < bufferLength; sample++) { Module.mOutputData[sample + offset] = data[(sample * speakerModeChannels) + channel]; } }},  
 4080797: function($0, $1, $2, $3) {const buffer = $0; const bufferLength = $1; const speakerModeChannels = $2; const frameFlag = $3; var data = HEAPF32.subarray((buffer / 4), (buffer / 4) + (bufferLength * speakerModeChannels)); const arrayOffset = speakerModeChannels * frameFlag * bufferLength; for (var channel = 0; channel < speakerModeChannels; channel++) { const offset = channel * bufferLength + arrayOffset; for (var sample = 0; sample < bufferLength; sample++) { Module.mOutputData[sample + offset] = data[(sample * speakerModeChannels) + channel]; } }},  
 4081328: function() {if (Module.mContext) { Module.mContext.suspend(); }},  
 4081384: function() {if (Module.mContext) { Module.mContext.resume(); }},  
 4081439: function() {var AudioContext = window.AudioContext || window.webkitAudioContext; var contextForCheck = new AudioContext(); if (!contextForCheck) { return 0; } var retValue = 0; if (self.AudioWorkletNode) { if (contextForCheck.audioWorklet.addModule) { retValue = 1; } } contextForCheck.close(); return retValue;},  
 4081743: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 4081804: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 4081868: function() {return Module.webglContextAttributes.powerPreference;},  
 4081926: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 4081981: function($0) {performance.now = function() { return $0; };},  
 4082029: function($0) {performance.now = function() { return $0; };},  
 4082077: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  function _Alert(messagePointer) {
  ...
  function _getTelegramUserData() {
          ...
          if (typeof window.Telegram !== 'undefined' && window.Telegram.WebApp) {
              ...
              if (initDataUnsafe && initDataUnsafe.user) {
                  const user = JSON.stringify(initDataUnsafe.user);
                  console.log("Sending user data to Unity: ", user);
  
                  if (UnityInstance !== 'undefined') {
                      UnityInstance.SendMessage('GameManager', 'OnTelegramUserDataReceived', user);
                  } else {
                      window.alert('Error fetching user info');
                      UnityInstance.SendMessage('GameManager', 'OnTelegramUserDataReceived', 'Error: Fetching User Info');
                  }
              } else {
                  console.error("No user data found.");
                  UnityInstance.SendMessage('GameManager', 'OnTelegramUserDataReceived', 'Error: No user data found.');
              }
  ...
  function _inviteTelegramFriends(referralCode, link) {
      var message = `Not your average tap-to-play game, Stables Bot 🏇🏻 will test your speed and precision as you play to reach your highest score and join weekly challenges to earn rewards!`;
      var code = UTF8ToString(referralCode);
      var linkstr = UTF8ToString(link);
      var linkToShare = `${linkstr}?startapp=${code}`;
  
      var isIos = /Mobi|iPhone|iPad|iPod/.test(navigator.userAgent);
  
      if (typeof window.Telegram !== 'undefined' && window.Telegram.WebApp) {
              if(isIos){
                  Telegram.WebApp.openTelegramLink(`https://t.me/share/url?url=${linkToShare}&text=${message}`);
              }else{
                  Telegram.WebApp.openLink(`https://t.me/share/url?text=${message}&url=${linkToShare}`);
              }
          } else {
              window.alert("Telegram Web App API is not available!");
          }
  }

  function _isDesktopBrowser() {
          // Use navigator.userAgent to detect if the device is a desktop
          var isDesktop = !/Mobi|Android|iPhone|iPad|iPod/.test(navigator.userAgent);
          return isDesktop ? 1 : 0; // Return 1 for desktop, 0 for mobile
      }
  ...
  function _showAd(blockId) {
      var bID = UTF8ToString(blockId);
      ...
      AdController.show().then((result) => {
          if(!bID.startsWith("int-")){
              const data = JSON.stringify(result);
              UnityInstance.SendMessage('GameManager', 'OnShowAdCompleted', data);
          }
      }).catch((result) => {
          UnityInstance.SendMessage('GameManager', 'OnShowAdNotCompleted');
      })
...
var asmLibraryArg = {
  ...
  "inviteTelegramFriends": _inviteTelegramFriends,
  ...
  "isDesktopBrowser": _isDesktopBrowser,
